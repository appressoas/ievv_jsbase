...
export default class WidgetRegistrySingleton {
    ...
    initializeWidget(element) {
        ...
        if (element.hasAttribute(this._widgetInstanceIdAttribute)) {
            const existingInstanceId = element.getAttribute(this._widgetInstanceIdAttribute);
            if (console.debug) {
                console.debug(
                    'Element already initialized as an ievv_jsbase widget:',
                    element,
                    'Skipping initialization. Perhaps you have more than one call to WidgetRegistrySingleton initializeAllWidgetsWithinElement(...)?'
                )
            }
            return this._widgetInstanceMap.get(existingInstanceId);
        }
